...
public class CommonMethods extends PageInitializer {
	...
	// In this class we develop functions and methods and develop functions that can
	...
	// 1st Function to send text, creat the mehtod sendText
	// identify element
	 ...
	 * @param textToSelect
	 */
	// handling dropdown and text in this method
	public static void selectDdValue(WebElement element, String textToSelect) {

		try {
			Select select = new Select(element);
			// I want to grab a collection of all options
			// and then loop through all options to get text and if match,
			// then we will be selection-->.getOptions
			List<WebElement> options = select.getOptions();

			for (WebElement el : options) {
				if (el.getText().equals(textToSelect)) {
					select.selectByVisibleText(textToSelect);
					break; // Selecting only if its avaible!!
	...
	/**
	 * Methods that accept alerts and catches exception if alert not present
	 */
	...
	public static void scrollDown(int pixel) {
		...
		/**
		 * Method that will scrool the page up based on the passed pixel parameters
		 */
	...
	try {
		FilUtils.copyFile(file, new File(filename));
	}catch(Exception ex) {
